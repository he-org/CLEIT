...
def train_dann(s_dataloaders, t_dataloaders, val_dataloader, test_dataloader, metric_name, **kwargs):
    """
    ...
    for epoch in range(int(kwargs['train_num_epochs'])):
        ...
        save_flag, stop_flag = model_save_check(history=target_regression_eval_val_history,
                                                ...
                                                tolerance_count=50)
        if save_flag:
            torch.save(target_regressor.state_dict(), os.path.join(kwargs['model_save_folder'], 'dann_regressor.pt'))
        if stop_flag:
            break
    target_regressor.load_state_dict(
        torch.load(os.path.join(kwargs['model_save_folder'], 'dann_regressor.pt')))
