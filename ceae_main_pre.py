...
def main(train_num_epochs=10000):
    train_fn = pretrain_ceae.train_ceae
    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    training_params = {
        'device': device,
        'model_save_folder': os.path.join('model_save', 'pre_ceae'),
    ...
    training_params.update(
        {
            'p_input_dim': data_provider.shape_dict['prot'],
            't_input_dim': data_provider.shape_dict['trans'],
