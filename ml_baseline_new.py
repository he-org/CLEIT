...
if __name__ == '__main__':
    ...
    labeled_samples = data_provider.get_labeled_samples()
    labeled_target_df = data_provider.target_df.loc[labeled_samples]
    label_trans_df = data_provider.trans_dat.loc[labeled_samples]
    label_prot_df = data_provider.prot_dat.loc[labeled_samples]

    train_trans_data = (label_trans_df, labeled_target_df)
    train_prot_data = (label_prot_df, labeled_target_df)

    trans_train_history, _ = n_time_cv_regress(train_trans_data, 'trans_pred', n=5, model_fn=regress_with_enet,
                                             test_data=None, random_state=2020)

    with open('./predictions/trans_pred.json', 'w') as f:
        json.dump(trans_train_history, f)

    prot_train_history, _ = n_time_cv_regress(train_prot_data, 'prot_pred', n=5, model_fn=regress_with_enet,
                                                            test_data=None, random_state=2020)

    with open('./predictions/prot_pred.json', 'w') as f:
        json.dump(prot_train_history, f)
